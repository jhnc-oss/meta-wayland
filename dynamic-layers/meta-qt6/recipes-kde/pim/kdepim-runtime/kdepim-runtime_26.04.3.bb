SUMMARY = "Akonadi agents and resources"
HOMEPAGE = "https://invent.kde.org/pim/kdepim-runtime"
LICENSE = "AGPL-3.0-or-later AND BSD-2-Clause AND BSD-3-Clause AND CC0-1.0 AND GPL-2.0-only AND GPL-2.0-or-later AND GPL-3.0-only AND GPL-3.0-or-later AND LGPL-2.0-only AND LGPL-2.0-or-later AND LGPL-2.1-or-later AND LGPL-3.0-only AND LGPL-3.0-or-later AND LicenseRef-KDE-Accepted-GPL AND LicenseRef-KDE-Accepted-LGPL"
LIC_FILES_CHKSUM = " \
    file://LICENSES/AGPL-3.0-or-later.txt;md5=a4af3f9f0c0fc9de318e4df46665906e \
    file://LICENSES/BSD-2-Clause.txt;md5=63d6ee386b8aaba70b1bf15a79ca50f2 \
    file://LICENSES/BSD-3-Clause.txt;md5=954f4d71a37096249f837652a7f586c0 \
    file://LICENSES/CC0-1.0.txt;md5=6fd064768b8d61c31ddd0540570fbd33 \
    file://LICENSES/GPL-2.0-only.txt;md5=93e64b4a83c7e441e48bbdeeea05c977 \
    file://LICENSES/GPL-2.0-or-later.txt;md5=fed54355545ffd980b814dab4a3b312c \
    file://LICENSES/GPL-3.0-only.txt;md5=1c76c4cc354acaac30ed4d5eefea7245 \
    file://LICENSES/GPL-3.0-or-later.txt;md5=1c76c4cc354acaac30ed4d5eefea7245 \
    file://LICENSES/LGPL-2.0-only.txt;md5=6d2d9952d88b50a51a5c73dc431d06c7 \
    file://LICENSES/LGPL-2.0-or-later.txt;md5=6d2d9952d88b50a51a5c73dc431d06c7 \
    file://LICENSES/LGPL-2.1-or-later.txt;md5=2a4f4fd2128ea2f65047ee63fbca9f68 \
    file://LICENSES/LGPL-3.0-only.txt;md5=c51d3eef3be114124d11349ca0d7e117 \
    file://LICENSES/LGPL-3.0-or-later.txt;md5=c51d3eef3be114124d11349ca0d7e117 \
    file://LICENSES/LicenseRef-KDE-Accepted-GPL.txt;md5=b4c280013bbbadfbe92219498dc5228c \
    file://LICENSES/LicenseRef-KDE-Accepted-LGPL.txt;md5=e4b79a181b6483b37d39a27f4d75e60a \
"

SRC_URI = "git://invent.kde.org/pim/kdepim-runtime.git;protocol=https;nobranch=1;tag=v26.04.3"
SRCREV = "83bfaa1c720abbc13b94e6c5d3d6dff07f8eec02"

DEPENDS = " \
    qtbase \
    qttools-native \
    extra-cmake-modules \
    libxslt-native \
    shared-mime-info \
    cyrus-sasl \
    qca \
    qtnetworkauth \
    kconfig \
    kconfigwidgets \
    knotifyconfig \
    kio \
    kitemmodels \
    kcodecs \
    kwindowsystem \
    knotifications \
    kcmutils \
    kdav \
    ktextaddons \
    akonadi \
    kmime \
    akonadi-mime \
    kmailtransport \
    kidentitymanagement \
    akonadi-contacts \
    kcontacts \
    kcalendarcore \
    kcalutils \
    kmbox \
    kimap \
    akonadi-calendar \
    libkgapi \
    kldap \
    pimcommon \
    qtkeychain \
    plasma-activities \
"

inherit kf6 gettext pkgconfig mime mime-xdg

PACKAGECONFIG ??= "${@bb.utils.contains('QTWEBENGINE_SUPPORTED', '1', 'webengine', '', d)}"
PACKAGECONFIG[webengine] = "-DCMAKE_DISABLE_FIND_PACKAGE_Qt6WebEngineWidgets=OFF,-DCMAKE_DISABLE_FIND_PACKAGE_Qt6WebEngineWidgets=ON,qtwebengine"

EXTRA_OECMAKE += "-DBUILD_TESTING=OFF -DOE_KF6_PATH_HOST_ROOT=${STAGING_DIR_HOST}"

FILES:${PN} += "${libdir}/plugins ${libdir}/qml ${datadir}"

RDEPENDS:${PN} += "akonadi"

do_install:append() {
	sed -i 's|#include "${S}/|#include "|' ${B}/resources/google-groupware/settingsadaptor.h
}
